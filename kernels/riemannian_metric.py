...
MATH:
    ...
    x both builds L and gets transformed by it):
        ...
        of a dense per-triangular-position unpack" reasoning):
            ...
            gate_k     = asinh(raw_gate_k / temperature)         temperature = sqrt(head_dim)
    ...
    backward (per head h, per token):
        ...
        off-diagonal branch (new):
            ...
            grad_raw_gate_k = grad_gate_k / (temperature * cosh(gate_k))     -- same "d(asinh)/d(raw) = sech"
...
def _riemannian_project_kernel(
    ...
    seq_len, heads, head_dim, rank, temperature,
    ...
    raw_gate_tile = tl.dot(x_block, w_tile, allow_tf32=False) / temperature  # (BLOCK_S, BLOCK_R), fp32 accumulate
...
def _riemannian_grad_per_token_kernel(
    ...
    lower_mask_ptr,
    seq_len, heads, head_dim, rank, temperature, rank_scale,
    ...
    if NUM_R_TILES == 1:
        ...
        for s in range(BLOCK_S):
            ...
            # gate = asinh(raw_gate/temperature) => d(gate)/d(raw_gate) =
            # 1/(temperature*cosh(gate)) -- same sech identity as before.
            grad_raw_gate_s = grad_gate_s * (2.0 / (tl.exp(gate_s) + tl.exp(-gate_s))) / temperature
        ...
        for r_tile in range(NUM_R_TILES):
            ...
            for s in range(BLOCK_S):
                ...
                grad_raw_gate_tile_s = grad_gate_tile_s * (2.0 / (tl.exp(gate_s) + tl.exp(-gate_s))) / temperature
...
class RiemannianMetricKernel(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, weight_diag, weight_W, weight_U, weight_V, temperature):
        ...
        BLOCK_R = max(16, min(128 // scale, triton.next_power_of_2(rank)))
        ...
        _riemannian_project_kernel[grid_proj](
            ...
            S, heads, head_dim, rank, temperature,
    ...
    def backward(ctx, grad_out):
        ...
        _riemannian_grad_per_token_kernel[grid_apply](
            ...
            lower_mask,
            S, heads, head_dim, rank, temperature, rank_scale,
