...
class RiemannianMetric(nn.Module):
    def __init__(self, heads: int, head_dim: int, rank: int, fused: bool = False):
        ...
        self.weight_W = nn.Parameter(torch.zeros(heads, head_dim, rank))
        self.weight_U = nn.Parameter(torch.empty(heads, head_dim, rank))
        self.weight_V = nn.Parameter(torch.empty(heads, head_dim, rank))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.fused:
            return RiemannianMetricKernel.apply(
                x, self.weight_diag, self.weight_W, self.weight_U, self.weight_V, self.temperature,
            )
        ...
        raw_gate = torch.einsum("nhsd,hdr->nhsr", x, self.weight_W) / self.temperature
        gate = torch.asinh(raw_gate)
